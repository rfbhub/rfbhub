...
function filterRoster(snapshot) {
  ...
  // Level filter
  if (ROSTER_FILTER.minLevel > 1) {
    raiders = raiders.filter(r => (r.level || 90) >= ROSTER_FILTER.minLevel);
  }

  // Item level filter
  if (ROSTER_FILTER.minIlvl > 0) {
    raiders = raiders.filter(r => getIlvl(r) >= ROSTER_FILTER.minIlvl);
  }

  // Parse filter: must appear in WCL data
  if (ROSTER_FILTER.requireParses && snapshot.wclData) {
    ...
    raiders = raiders.filter(r => parsedNames.has(r.name.toLowerCase()));
  }
