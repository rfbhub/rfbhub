...
function filterRoster(snapshot) {
  ...
  // Build set of players with mythic WCL parses (difficulty 5)
  const mythicParsedNames = new Set();
  if (snapshot.wclData?.difficulties?.["5"]?.raiders) {
    for (const r of snapshot.wclData.difficulties["5"].raiders) {
      mythicParsedNames.add(r.name.toLowerCase());
    }
  }

  // Filter: has mythic parses (bypasses ilvl, handles PvP gear) OR meets ilvl minimum
  // OR is tagged as an M+ roster player (3k+ IO guild member)
  const mplusThreshold = CONFIG.mplusThreshold || 3000;
  raiders = raiders.filter(r => {
    if (mythicParsedNames.has(r.name.toLowerCase())) return true;
    if (getIlvl(r) >= ROSTER_FILTER.minIlvl) return true;
    // Include M+ roster players (tagged by update-roster.js or by IO threshold)
    if (r._mplusOnly) return true;
    if (getRio(r) >= mplusThreshold) return true;
    return false;
  });
...
function generateRecap(oldSnap, newSnap) {
  ...
  // Save unfiltered raiders for "New Faces" check before filtering
  oldSnap._unfilteredRaiders = oldSnap.raiders || [];
  newSnap._unfilteredRaiders = newSnap.raiders || [];
  ...
  // Keep unfiltered old roster for "New Faces" detection.
  // A player is only truly new if they weren't in the guild at all before,
  // not just because they crossed a filter threshold.
  const unfilteredOldNames = new Set((oldSnap._unfilteredRaiders || []).map(r => r.name.toLowerCase()));
  ...
  // Only show individual progression when it differs from the guild-level change.
  // When the whole guild kills a new boss together, everyone gets the same
  // X/9 -> Y/9 change, which is redundant with the guildProgression section.
  // We only surface players whose individual change is DIFFERENT from the guild
  // (e.g., someone pugged ahead, an alt caught up, or a trial joined mid-tier).
  const guildMythicGain = (newGuildProg?.mythic_bosses_killed || 0) - (oldGuildProg?.mythic_bosses_killed || 0);
  const progChanges = [];
  for (const name of common) {
    ...
    const playerGain = newMythic - oldMythic;
    if (playerGain > 0 && playerGain !== guildMythicGain) {
      progChanges.push({
        name: newRaiders[name].name,
        class: newRaiders[name].class,
  ...
  // ── New Faces ──
  // Check against UNFILTERED old roster so long-time members who just
  // crossed a filter threshold don't get flagged as "new"
  const newFaces = [];
  for (const name of newNames) {
    if (unfilteredOldNames.has(name)) continue;
  ...
  const raidOnly = newSnap.raiders.filter(r => !r._mplusOnly);
  const oldRaidOnly = oldSnap.raiders.filter(r => !r._mplusOnly);
  const totalRaiders = raidOnly.length;
  const oldTotalRaiders = oldRaidOnly.length;
  const avgRio = Math.round(
    raidOnly.reduce((s, r) => s + getRio(r), 0) / totalRaiders
  );
  const oldAvgRio = Math.round(
    oldRaidOnly.reduce((s, r) => s + getRio(r), 0) / oldTotalRaiders
  );
  const avgIlvl = round1(
    raidOnly.reduce((s, r) => s + getIlvl(r), 0) / totalRaiders
  );
  const oldAvgIlvl = round1(
    oldRaidOnly.reduce((s, r) => s + getIlvl(r), 0) / oldTotalRaiders
  );
