...
async function fetchGuildProfile(retries = 3) {
  ...
  for (let attempt = 0; attempt <= retries; attempt++) {
    ...
    if (r.status < 500 || attempt === retries) {
      throw new Error(`Raider.IO guild fetch failed: ${r.status}`);
    }
    console.log(`  Guild fetch returned ${r.status}, retrying in 5s... (${attempt + 1}/${retries})`);
    await sleep(5000);
  }
}
...
async function main() {
  ...
  const mplusCandidates = guildMembers.filter(m => {
    ...
    // R.IO guild member data doesn't include level reliably,
    // so use active_spec_name + last_crawled_at as a proxy for active max-level chars
    if (!char.active_spec_name) return false;
    const crawled = new Date(char.last_crawled_at);
    const cutoff = new Date(); cutoff.setDate(cutoff.getDate() - 60);
    return crawled > cutoff;
  });
