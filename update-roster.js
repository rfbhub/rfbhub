...
async function fetchGuildRoster() {
  const url = `https://raider.io/api/v1/guilds/profile?region=${GUILD_REGION}&realm=${encodeURIComponent(GUILD_REALM)}&name=${encodeURIComponent(GUILD_NAME)}&fields=members,raid_progression`;
...
async function main() {
  ...
  // 5. Build roster.json
  // Guild object: only include what the site needs (name, raid_progression)
