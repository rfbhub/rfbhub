...
async function main() {
  ...
  // 4. Apply roster filter: raider = has mythic parses OR ilvl >= minimum.
  // Mythic parses bypass the ilvl check (handles PvP gear logout).
  let mythicParsedNames = new Set();
  if (fs.existsSync('wcl-data.json')) {
    try {
      const wclData = JSON.parse(fs.readFileSync('wcl-data.json', 'utf8'));
      // Only collect names from mythic difficulty (5)
      const mythicRaiders = wclData.difficulties?.["5"]?.raiders || [];
      for (const r of mythicRaiders) mythicParsedNames.add(r.name.toLowerCase());
      if (wclData.partitions) {
        for (const p of Object.values(wclData.partitions)) {
          const pr = p.difficulties?.["5"]?.raiders || [];
          for (const r of pr) mythicParsedNames.add(r.name.toLowerCase());
        }
      }
      console.log(`Loaded WCL data: ${mythicParsedNames.size} mythic raider names`);
    } catch (e) {
  ...
  const filtered = allChars.filter(c => {
    const hasMythicKill = mythicParsedNames.has(c.name.toLowerCase());
    if (hasMythicKill) return true;
