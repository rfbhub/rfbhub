...
async function main() {
  ...
  // 2. Filter down to actual raiders (has raid kills on current tier)
  const raiderMembers = (guild.members || []).filter(m => {
    const prog = m.character?.raid_progression?.[RAID_TIER];
    return prog && raidScore(prog) > 0;
  });
  console.log(`Filtered to ${raiderMembers.length} raiders with kills on ${RAID_TIER}`);
  console.log('');

  // 3. Fetch per-character details
  console.log('Fetching character details...');
  const raiders = [];
  for (let i = 0; i < raiderMembers.length; i++) {
    const m = raiderMembers[i];
    const name = m.character.name;
    const realm = m.character.realm;
    console.log(`[${i + 1}/${raiderMembers.length}] ${name} @ ${realm}`);
    const details = await fetchCharacterDetails(name, realm);
    if (details) raiders.push(details);
    await sleep(120); // be polite to R.IO
  }
  console.log('');
  console.log(`Collected details for ${raiders.length} raiders`);
