...
async function main() {
  ...
  // 4. Apply roster filter: ilvl + parses
  // Players with WCL parses bypass the ilvl check (handles PvP gear edge case)
  ...
  const filteredChars = characters.filter(c => {
    if (parsedNames.has(c.name.toLowerCase())) return true; // has parses = keep regardless of ilvl
    if (ROSTER_FILTER.requireParses) return false; // no parses + parses required = out
