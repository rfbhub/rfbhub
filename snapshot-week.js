...
async function main() {
  ...
  // 2. Fetch per-character details for every max-level member.
  // We can't filter on raid_progression up front because the guild/profile
  // endpoint doesn't include per-character progression. We have to fetch each
  // character and filter after.
  const members = (guild.members || []).filter(m => (m.character?.level || 0) >= 80);
  console.log(`Fetching character details for ${members.length} max-level members...`);
