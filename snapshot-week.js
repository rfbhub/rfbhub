...
async function main() {
  ...
  // 3. Grab the current WCL cache (need it for roster filter + snapshot)
  let wclData = null;
  ...
  // 4. Apply roster filter: has mythic parses (WCL) OR meets ilvl minimum.
  // Mythic parses bypass ilvl check (handles PvP gear logout).
  const mythicParsedNames = new Set();
  if (wclData?.difficulties?.["5"]?.raiders) {
    for (const r of wclData.difficulties["5"].raiders) {
      mythicParsedNames.add(r.name.toLowerCase());
    }
  }
  const beforeFilter = characters.length;
  const filteredChars = characters.filter(c => {
    if (mythicParsedNames.has(c.name.toLowerCase())) return true;
    const ilvl = c.gear?.item_level_equipped || 0;
    return ilvl >= ROSTER_FILTER.minIlvl;
  });
  characters.length = 0;
  characters.push(...filteredChars);
  console.log(`Roster filter: ${beforeFilter} -> ${characters.length} characters`);

  // 5. Filter down to actual raiders (has kills on the current tier).
