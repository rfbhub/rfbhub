...
async function main() {
  ...
  for (let i = 0; i < members.length; i++) {
    ...
    // Level comes from the guild roster (character profile doesn't include it)
    const memberLevel = m.character.level || 0;
    if (memberLevel < ROSTER_FILTER.minLevel) {
      console.log(`  Skipped: level ${memberLevel} < ${ROSTER_FILTER.minLevel}`);
      continue;
    }

    const details = await fetchCharacterDetails(name, realm);
    if (details) {
      // Apply roster filter: ilvl
