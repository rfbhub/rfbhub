...
const WCL_PARTITION = CONFIG.wclPartition || null;
...
// Active partition, auto-detected at runtime. Starts null (use WCL default),
// falls back to CONFIG.wclPartition if the default partition has no data.
let activePartition = null;

async function fetchWCLZoneRankings(name, serverSlug, serverRegion, difficulty, metric, partitionOverride) {
  const token = await getWCLToken();
  const m = metric === 'hps' ? 'hps' : 'dps';
  const p = partitionOverride !== undefined ? partitionOverride : activePartition;
...
async function main() {
  ...
  // Auto-detect partition: probe the first raider with no partition (WCL default).
  // If empty, fall back to CONFIG.wclPartition (previous patch's data).
  console.log('Detecting WCL partition...');
  const probeChar = raiders[0];
  const probeSlug = (probeChar.realm || 'stormrage').toLowerCase().replace(/\s+/g, '-');
  const probeMetric = isHealerSpec(probeChar.active_spec_name) ? 'hps' : 'dps';
  const probeResult = await fetchWCLZoneRankings(probeChar.name, probeSlug, 'US', 4, probeMetric, null);
  ...
  if (probeHasData) {
    ...
    console.log(`Default partition empty, falling back to partition ${WCL_PARTITION}`);
  } else {
    activePartition = null;
    console.log('Default partition empty and no fallback partition configured');
  }
