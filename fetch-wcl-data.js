  ...
  for (const raiderData of allRaiderData) {
    ...
    raidersWithParses.push({
      ...
      bosses: rankings.map(r => {
        ...
        // Preserve alt-merge source so the front end links to the right WCL page
...
function detectEvents(prevData, newData, characters, activity) {
  ...
  // Track previous pink/orange parses
    ...
    for (const raider of raiders) {
      ...
      for (const boss of raider.bosses || []) {
        ...
        if (boss.parse >= 99 && !prevPinkParses.has(key)) {
          ...
          addEvent(activity, 'orange_parse', '🧡',
            `${raider.name} logged a ${boss.parse}% ${diffNames[diffId]} parse on ${boss.name}`,
            `${diffNames[diffId]} ${boss.name}`
          );
        }
...
async function main() {
  ...
  // Fetch raider list: WowAudit (curated) first, guild roster fallback
  const waRoster = await fetchWowAuditRoster();

  let raiders;

  if (waRoster && waRoster.length > 0) {
    // WowAudit path: fetch R.IO details for each curated raider
    console.log('');
    ...
    // Fallback: guild roster + raid kill filter
  ...
  for (const part of WCL_PARTITIONS) {
    ...
    for (let i = 0; i < raiders.length; i++) {
      ...
      // Metric detection: fetch dps first, then check the spec WCL actually reports.
      // If WCL says they raided as a healer spec, re-fetch with hps.
      // This is the only reliable way since R.IO's active spec can be wrong.
      ...
      for (const diff of DIFFICULTIES) {
        ...
        // Check if WCL's reported spec is a healer but we fetched dps (or vice versa).
        // The spec field in each ranking tells us what they actually played as.
        if (rankings.length > 0) {
          ...
          if (wclIsHealer && !weUsedHps) {
            // WCL says healer but we fetched dps, re-fetch with hps
            ...
            // WCL says dps but we fetched hps, re-fetch with dps
      ...
      // Fetch supplemental zones (e.g. Sporefall) and append rankings
      for (const supZone of SUPPLEMENTAL_ZONES) {
        for (const diff of DIFFICULTIES) {
          ...
          // Healer spec correction for supplemental zone
          if (rankings.length > 0) {
            const wclSpec = rankings[0].spec;
            const wclIsHealer = isHealerSpec(wclSpec);
            const weUsedHps = metric === 'hps';
    ...
    // ── Alt Merge: fetch alt WCL data and merge best parses into main ──
    for (const merge of ALT_MERGE) {
      ...
      for (const alt of merge.alts) {
        ...
        // Use same metric as the main character
        ...
        for (const diff of DIFFICULTIES) {
          ...
          // Same healer spec correction as the main loop
          ...
          // Ensure main has a difficulty entry
          ...
          for (const altBoss of altRankings) {
            ...
            // Tag with source character so the front end can link to the right WCL page
            const tagged = { ...altBoss, _srcChar: alt.name, _srcRealm: altServerSlug };

            if (!mainBoss) {
              // Main doesn't have this boss at all, add it
              mainRankings.push(tagged);
              merged++;
            } else if (altBoss.rankPercent > mainBoss.rankPercent) {
              // Alt has a better parse, replace
        ...
        // Also fetch supplemental zones for the alt
        for (const supZone of SUPPLEMENTAL_ZONES) {
          for (const diff of DIFFICULTIES) {
        ...
        // Remove the alt from allRaiderData if they happen to be there
        // (shouldn't be if they're not on WowAudit, but just in case)
