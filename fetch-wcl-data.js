...
async function main() {
  ...
  for (const part of WCL_PARTITIONS) {
    ...
    for (let i = 0; i < raiders.length; i++) {
      ...
      // Fetch both dps and hps metrics, keep whichever gives a better result per
      // difficulty. This handles dual-spec players and cases where R.IO's active
      // spec doesn't match the spec they actually raid as.
      const primaryMetric = isHealerSpec(char.active_spec_name) ? 'hps' : 'dps';
      const altMetric = primaryMetric === 'hps' ? 'dps' : 'hps';

      const raiderData = {
        name: char.name,
        class: char.class,
        spec: char.active_spec_name || null,
        role: primaryMetric === 'hps' ? 'healer' : 'dps',
        realm: serverSlug,
        difficulties: {}
      };

      for (const diff of DIFFICULTIES) {
        // Fetch primary metric
        const primary = await fetchWCLZoneRankings(char.name, serverSlug, 'US', diff.id, primaryMetric, partId);
        await sleep(150);

        // Fetch alternate metric
        const alt = await fetchWCLZoneRankings(char.name, serverSlug, 'US', diff.id, altMetric, partId);
        await sleep(150);

        // Pick whichever metric returned a higher average parse
        const primaryRankings = (primary?.rankings || []).filter(r => r.rankPercent);
        const altRankings = (alt?.rankings || []).filter(r => r.rankPercent);
        const primaryAvg = primaryRankings.length > 0 ? primaryRankings.reduce((s, r) => s + r.rankPercent, 0) / primaryRankings.length : 0;
        const altAvg = altRankings.length > 0 ? altRankings.reduce((s, r) => s + r.rankPercent, 0) / altRankings.length : 0;

        const bestRankings = altAvg > primaryAvg ? altRankings : primaryRankings;
        const usedMetric = altAvg > primaryAvg ? altMetric : primaryMetric;

        if (bestRankings.length > 0) {
          raiderData.difficulties[diff.id] = {
            rankings: bestRankings
          };
          const metricNote = usedMetric !== primaryMetric ? ` (used ${usedMetric})` : '';
