...
async function main() {
  ...
  // Build set of players with existing WCL parses (from previous run).
  // Players with parses bypass the ilvl filter (handles PvP gear edge case).
  const prevParsedNames = new Set();
  if (prevData?.difficulties) {
    for (const diffId of Object.keys(prevData.difficulties)) {
      for (const r of prevData.difficulties[diffId]?.raiders || []) {
        prevParsedNames.add(r.name.toLowerCase());
      }
    }
  }

  // Apply roster filter: ilvl check, but skip it for players with known parses
  const filtered = characters.filter(c => {
    if (prevParsedNames.has(c.name.toLowerCase())) return true;
    const ilvl = c.gear?.item_level_equipped || 0;
    if (ilvl < ROSTER_FILTER.minIlvl) {
      console.log(`  Skipped ${c.name}: ilvl ${ilvl} < ${ROSTER_FILTER.minIlvl}`);
      return false;
    }
    return true;
  });
  ...
  console.log(`After roster filter: ${characters.length} characters`);
