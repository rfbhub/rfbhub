...
async function main() {
  ...
  for (const part of WCL_PARTITIONS) {
    ...
    // Auto-detect default difficulty: Normal until 3+ unique boss kills on Heroic, then Heroic until 3+ Mythic
    const uniqueBosses = diffId => {
      const s = new Set();
      for (const r of difficultyData[diffId].raiders) for (const b of r.bosses) s.add(b.name);
      return s.size;
    };
    let defaultDifficulty = 3;
    if (uniqueBosses(5) >= 3) defaultDifficulty = 5;
    else if (uniqueBosses(4) >= 3) defaultDifficulty = 4;
