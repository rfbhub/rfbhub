...
async function main() {
  ...
  for (const part of WCL_PARTITIONS) {
    ...
    for (let i = 0; i < raiders.length; i++) {
      ...
      // Healer classes can raid as DPS too (e.g. Evoker: Preservation vs Augmentation).
      // If R.IO says they're a healer, trust it and use hps.
      // If R.IO says they're DPS but the class HAS a healer spec, also fetch hps
      // in case R.IO's active spec is wrong, and only use hps if dps returns nothing.
      // Pure DPS classes (Mage, Warlock, Rogue, Hunter) never fetch hps.
      const HEALER_CLASSES = new Set(['Priest', 'Paladin', 'Shaman', 'Monk', 'Druid', 'Evoker']);
      const isHealer = isHealerSpec(char.active_spec_name);
      const hasHealerSpec = HEALER_CLASSES.has(char.class);
      ...
      const raiderData = {
        ...
        role: isHealer ? 'healer' : 'dps',
        realm: serverSlug,
        difficulties: {}
      };

      for (const diff of DIFFICULTIES) {
        // Always fetch with the primary metric
        const primary = await fetchWCLZoneRankings(char.name, serverSlug, 'US', diff.id, metric, partId);
        await sleep(150);

        const primaryRankings = (primary?.rankings || []).filter(r => r.rankPercent);

        // Only try the alt metric if:
        // 1. The class has a healer spec (hybrid class)
        // 2. R.IO says DPS but primary returned no data (they might actually be healing)
        let bestRankings = primaryRankings;
        let usedMetric = metric;

        if (!isHealer && hasHealerSpec && primaryRankings.length === 0) {
          const alt = await fetchWCLZoneRankings(char.name, serverSlug, 'US', diff.id, 'hps', partId);
          await sleep(150);
          const altRankings = (alt?.rankings || []).filter(r => r.rankPercent);
          if (altRankings.length > 0) {
            bestRankings = altRankings;
            usedMetric = 'hps';
            raiderData.role = 'healer'; // Correct the role
          }
        }

        if (bestRankings.length > 0) {
          raiderData.difficulties[diff.id] = {
            rankings: bestRankings
          };
          const metricNote = usedMetric !== metric ? ` (used ${usedMetric})` : '';
          console.log(`    ${diff.name}: ${bestRankings.length} bosses${metricNote}`);
        }
      }
